...
struct property_wrapper_definition_generator
{
   ...
   bool generate(OutputIterator sink, attributes::property_def const& property, Context context) const
   {
      ...
      auto get_params = property.get_keys.size() + property.get_values.size();
      auto set_params = property.set_keys.size() + property.set_values.size();

      // C# properties must have a single value.
      if (get_params > 1 || set_params > 1)
        return true;

      // FIXME Currently supporting properties with both getter and setter.
      ...
      std::string dir_mod = direction_modifier(setter.parameters[0]);

      if (!as_generator(scope_tab << "public " << type(true) << " " << managed_name << " {\n"
                        << scope_tab << scope_tab << "get { return Get" << managed_name << "(); }\n"
                        << scope_tab << scope_tab << "set { Set" << managed_name << "(" << dir_mod << "value); }\n"
                        << scope_tab << "}\n"
            ).generate(sink, getter.return_type, context))
         return false;
